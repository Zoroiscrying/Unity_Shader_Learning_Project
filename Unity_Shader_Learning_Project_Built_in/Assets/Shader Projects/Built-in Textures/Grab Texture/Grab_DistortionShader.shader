Shader "Zoroiscrying/GrabTexture/Distortion"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed2 movingUV = fixed2(0, 1 * _Time.x);
                float2 distortion = UnpackNormal(tex2D(_DistortionGuide, i.distortionUV+movingUV)).xy;
                distortion *= _DistortionAmount * mask * i.color.a;
                i.grabPassUV.xy += distortion * i.grabPassUV.z;
                fixed4 col = tex2Dproj(_GrabTexture, i.grabPassUV) + fixed4(0.1, 0.1, 0.1, 0);
                return col;
            }
            ENDCG
        }
